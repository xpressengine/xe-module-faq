...
class faqView extends faq
{
	...
	function dispFaqContentView()
	{
		...
		if($question_srl)
		{
			...
			{
				// compare module_srl
				if($oQuestion->get('module_srl') != $this->module_info->module_srl )
				{
	...
	function dispFaqDelete()
	{
		...
		if($oQuestion && !$oQuestion->isExists())
		{
			return $this->dispFaqContent();
		}
	...
	{
		$script =  sprintf('<script type="text/javascript"> jQuery(function()
